...
def receive_from_end(endSocket):
    ...
    if b'Content-Length' not in temp_message:
        ...
        content_length = extract_content_length(temp_header) - sys.getsizeof(temp_message[end_of_header:])
        print("!!!", content_length, temp_header)
        while content_length > 0:
            temp_message = endSocket.recv(content_length)
            end_of_header = temp_message.find(b'\r\n\r\n') + len(b'\r\n\r\n')
            temp_header = temp_message[:end_of_header].decode('utf-8', 'ignore')
            content_length = content_length - sys.getsizeof(temp_message[end_of_header:])
